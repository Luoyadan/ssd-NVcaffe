...
__global__ void PermuteKernel_B(const int nthreads,
    Btype* const bottom_data, const bool forward, const int* permute_order,
    const int* old_steps, const int* new_steps, const int num_axes,
    Btype* const top_data) {
...
template <typename Ftype, typename Btype>
__global__ void PermuteKernel_F(const int nthreads,
    Ftype* const bottom_data, const bool forward, const int* permute_order,
    const int* old_steps, const int* new_steps, const int num_axes,
...
void PermuteLayer<Ftype, Btype>::Forward_gpu(const vector<Blob*>& bottom,
  ...
  if (need_permute_) {
    Ftype* bottom_data = bottom[0]->mutable_gpu_data<Ftype>();
    ...
    PermuteKernel_F<Ftype, Btype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
...
void PermuteLayer<Ftype, Btype>::Backward_gpu(const vector<Blob*>& top,
  ...
  if (need_permute_) {
    Btype* top_diff = top[0]->mutable_gpu_diff<Btype>();
    ...
    bool foward = false;
    // NOLINT_NEXT_LINE(whitespace/operators)
    PermuteKernel_B<Ftype, Btype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
