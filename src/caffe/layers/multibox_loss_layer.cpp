...
void MultiBoxLossLayer<Ftype, Btype>::LayerSetUp(const vector<Blob*>& bottom,
  ...
  if (conf_loss_type_ == MultiBoxLossParameter_ConfLossType_SOFTMAX) {
    ...
    layer_param.add_loss_weight(Ftype(1.));
...
void MultiBoxLossLayer<Ftype, Btype>::Forward_cpu(const vector<Blob*>& bottom,
    const vector<Blob*>& top) {
  const Ftype* loc_data = bottom[0]->template cpu_data<Ftype>();
  const Ftype* conf_data = bottom[1]->template cpu_data<Ftype>();
  const Ftype* prior_data = bottom[2]->template cpu_data<Ftype>();
  const Ftype* gt_data = bottom[3]->template cpu_data<Ftype>();
  ...
  if (num_matches_ >= 1) {
    ...
    loc_pred_.Reshape(loc_shape);
    loc_gt_.Reshape(loc_shape);
    Ftype* loc_pred_data = loc_pred_.template mutable_cpu_data<Ftype>();
    Ftype* loc_gt_data = loc_gt_.template mutable_cpu_data<Ftype>();
    ...
    loc_loss_.template mutable_cpu_data<Ftype>()[0] = 0;
  }
  ...
  if (num_conf_ >= 1) {
    ...
    Ftype* conf_pred_data = conf_pred_.template mutable_cpu_data();
    Ftype* conf_gt_data = conf_gt_.template mutable_cpu_data();
    caffe_set<Ftype>(conf_gt_.count(), Ftype(background_label_id_), conf_gt_data);
    ...
    conf_loss_.template mutable_cpu_data()[0] = 0;
  }

  top[0]->template mutable_cpu_data<Ftype>()[0] = 0;
  if (this->layer_param_.propagate_down(0)) {
    Ftype normalizer = LossLayer<Ftype, Btype>::GetNormalizer_F(
        normalization_, num_, num_priors_, num_matches_);
    top[0]->template mutable_cpu_data<Ftype>()[0] +=
        loc_weight_ * loc_loss_.template cpu_data<Ftype>()[0] / normalizer;
  }
  if (this->layer_param_.propagate_down(1)) {
    Ftype normalizer = LossLayer<Ftype, Btype>::GetNormalizer_F(
        normalization_, num_, num_priors_, num_matches_);
    top[0]->template mutable_cpu_data<Ftype>()[0] += conf_loss_.template cpu_data<Ftype>()[0] / normalizer;
  }
}
...
void MultiBoxLossLayer<Ftype, Btype>::Backward_cpu(const vector<Blob*>& top,
  ...
  if (propagate_down[0]) {
    Btype* loc_bottom_diff = bottom[0]->template mutable_cpu_diff<Btype>();
    caffe_set<Btype>(bottom[0]->count(), Btype(0), loc_bottom_diff);
    if (num_matches_ >= 1) {
      ...
      loc_loss_layer_->Backward(loc_top_vec_, loc_propagate_down,
                                loc_bottom_vec_);
      // Scale gradient.
      Btype normalizer = LossLayer<Ftype, Btype>::GetNormalizer_B(
          normalization_, num_, num_priors_, num_matches_);
      Btype loss_weight = top[0]->template cpu_diff<Btype>()[0] / normalizer;
      caffe_scal<Btype>(loc_pred_.count(), loss_weight, loc_pred_.template mutable_cpu_diff<Btype>());
      // Copy gradient back to bottom[0].
      const Btype* loc_pred_diff = loc_pred_.template cpu_diff<Btype>();
      int count = 0;
      for (int i = 0; i < num_; ++i) {
        for (map<int, vector<int> >::iterator it =
          ...
          for (int j = 0; j < match_index.size(); ++j) {
            ...
            caffe_copy<Btype>(4, loc_pred_diff + count * 4,
  ...
  if (propagate_down[1]) {
    Btype* conf_bottom_diff = bottom[1]->template mutable_cpu_diff<Btype>();
    caffe_set<Btype>(bottom[1]->count(), Btype(0), conf_bottom_diff);
    if (num_conf_ >= 1) {
      ...
      Btype normalizer = LossLayer<Ftype, Btype>::GetNormalizer_B(
          normalization_, num_, num_priors_, num_matches_);
      Btype loss_weight = top[0]->template cpu_diff<Btype>()[0] / normalizer;
      caffe_scal<Btype>(conf_pred_.count(), loss_weight,
                 conf_pred_.template mutable_cpu_diff<Btype>());
      // Copy gradient back to bottom[1].
      const Btype* conf_pred_diff = conf_pred_.template cpu_diff<Btype>();
      if (do_neg_mining_) {
        int count = 0;
        for (int i = 0; i < num_; ++i) {
