...
std::mutex AnnodataReader::DataCache::cache_mutex_;
unique_ptr<AnnodataReader::DataCache> AnnodataReader::DataCache::data_cache_inst_;

AnnodataReader::AnnodataReader(const LayerParameter& param,
    size_t solver_count,
    size_t solver_rank,
  ...
  for (size_t i = 0; i < queues_num_; ++i) {
    full_[i] = make_shared<BlockingQueue<shared_ptr<AnnotatedDatum>>>();
    free_[i] = make_shared<BlockingQueue<shared_ptr<AnnotatedDatum>>>();
    for (size_t j = 0; j < queue_depth_ - 1U; ++j) {  // +1 in InternalThreadEntryN
      free_[i]->push(make_shared<AnnotatedDatum>());
    }
  }
  db_source_ = param.data_param().source();
  init_ = make_shared<BlockingQueue<shared_ptr<AnnotatedDatum>>>();
  StartInternalThread(false, Caffe::next_seed());
}

AnnodataReader::~AnnodataReader() {
  StopInternalThread();
}

void AnnodataReader::InternalThreadEntry() {
  InternalThreadEntryN(0U);
}

void AnnodataReader::InternalThreadEntryN(size_t thread_id) {
  if (cache_) {
  ...
  shared_ptr<AnnotatedDatum> init_datum = make_shared<AnnotatedDatum>();
  ...
  shared_ptr<AnnotatedDatum> datum = make_shared<AnnotatedDatum>();
  try {
...
shared_ptr<AnnotatedDatum>& AnnodataReader::DataCache::next_new() {
  std::lock_guard<std::mutex> lock(cache_mutex_);
  cache_buffer_.emplace_back(make_shared<AnnotatedDatum>());
  return cache_buffer_.back();
}

shared_ptr<AnnotatedDatum>& AnnodataReader::DataCache::next_cached() {
  ...
  shared_ptr<AnnotatedDatum>& datum = cache_buffer_[cache_idx_++];
...
void AnnodataReader::DataCache::just_cached() {
  just_cached_.store(true);
  std::lock_guard<std::mutex> lock(cache_mutex_);
  cached_flags_[std::this_thread::get_id()]->set();
}

bool AnnodataReader::DataCache::check_memory() {
  std::lock_guard<std::mutex> lock(cache_mutex_);
  bool mem_ok = true;
...
AnnodataReader::CursorManager::CursorManager(shared_ptr<db::DB> db, AnnodataReader* reader,
...
AnnodataReader::CursorManager::~CursorManager() {
  cursor_.reset();
  db_->Close();
}
...
S1 |                          r1pt1.q1                    --> S1.tr0 S1.q2
   ...
   |                                                      --> S1.tr1 S1.q4
      ...
      <---------- full cycle ----------->
*/
void AnnodataReader::CursorManager::rewind() {
...
void AnnodataReader::CursorManager::fetch(AnnotatedDatum* datum) {
